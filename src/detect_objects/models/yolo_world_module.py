...
from .model_config import DEFAULT_MODELS_CONFIG_PATH, load_yolo_world_config
...
class YOLO_World_Manager:
    ...
    def __init__(
        ...
        self._model_path = (
            Path(model_path).expanduser() if model_path else config.weights
        )
    ...
    def cache_class_embeddings(self, classes: Sequence[str]) -> None:
        ...
        stored_indexes = {
            class_name: index for index, class_name in enumerate(stored_names)
        }
        missing_classes = [
        ...
        if missing_classes:
            ...
            stored_indexes = {
                class_name: index for index, class_name in enumerate(normalized_classes)
            }

        if stored_features.shape[1] < len(stored_indexes):
            raise RuntimeError("클래스 개수와 텍스트 임베딩 개수가 일치하지 않습니다")
